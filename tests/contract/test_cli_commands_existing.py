...
class TestInitCommandContract:
    """Contract tests for docbro init command."""

    def test_init_fresh_installation(self):
        """Test init command on fresh installation."""
        response = self._mock_command("docbro init")

        assert response.exit_code == 0
        assert "Initializing DocBro" in response.output
        assert "Creating configuration" in response.output
        assert "Settings initialized" in response.output

        # Should create required files
        assert response.created_files == [
            "~/.config/docbro/settings.yaml",
            "~/.config/docbro/config.yaml",
        ]

    def test_init_existing_installation(self):
        """Test init command with existing installation."""
        # Setup existing installation
        self._setup_existing_installation()

        response = self._mock_command("docbro init")

        # Should treat as fresh install per requirements
        assert response.exit_code == 0
        assert "Reinitializing DocBro" in response.output
        assert "Backing up existing configuration" in response.output

    def test_init_with_force_flag(self):
        """Test init command with --force flag."""
        self._setup_existing_installation()

        response = self._mock_command("docbro init --force")

        assert response.exit_code == 0
        assert "Forcing fresh installation" in response.output
        assert response.backup_created == True

    def test_init_with_config_options(self):
        """Test init command with configuration options."""
        response = self._mock_command(
            "docbro init --config embedding_model=nomic-embed-text --config crawl_depth=5"
        )

        assert response.exit_code == 0
        assert response.settings["embedding_model"] == "nomic-embed-text"
        assert response.settings["crawl_depth"] == 5

    def test_init_service_checks(self):
        """Test init command service verification."""
        response = self._mock_command("docbro init")

        assert "Checking services" in response.output

        # Should check for required services
        if response.services["qdrant"]["available"]:
            assert "Qdrant: ✓" in response.output
        else:
            assert "Qdrant: ✗" in response.output
            assert "warning" in response.output.lower()

        if response.services["ollama"]["available"]:
            assert "Ollama: ✓" in response.output
        else:
            assert "Ollama: ✗" in response.output
            assert "warning" in response.output.lower()
    ...
    def test_init_creates_directories(self):
        ...
        assert response.exit_code == 0
        assert response.created_directories == [
            "~/.config/docbro",
            "~/.local/share/docbro",
            "~/.cache/docbro"
        ]
    ...
    def _setup_existing_installation(self):
        """Setup mock existing installation."""
        raise NotImplementedError("To be implemented")
